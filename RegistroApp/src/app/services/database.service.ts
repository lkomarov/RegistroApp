...
export class DatabaseService {
  ...
  async findUser(userName: string, password: string): Promise<User | undefined> {
    try {
      ...
      if (rows && rows.length > 0) {
        return this.rowToUser(rows[0]);  // Caso 1: Usuario encontrado
      } else {
        console.warn('No se encontró al ussario:', userName);
        return undefined;  // Caso 2: No se encontró el usuario
      }
    } catch (error) {
  ...
  async findUserByEmail(email: string): Promise<User | undefined> {
    try {
      ...
      // Verifica que 'rows' no sea undefined y contenga al menos un elemento
      if (rows && rows.length > 0) {
        return this.rowToUser(rows[0]);  // Caso 1: Usuario encontrado
      } else {
      ...
      showAlertError('DataBaseService.findUserByEmail', error);
      return undefined;  // Caso 3: Error al buscar el usuario
    }
  }
